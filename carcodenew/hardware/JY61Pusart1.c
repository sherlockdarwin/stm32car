...
static uint8_t RxBuffer[11]; // 存放一帧数据
static uint8_t imu_rx_cnt = 0;    // 帧解析计数器(与路点计数 waypoint_count 解耦)
volatile int16_t Yaw;          // 解析出的航向角(原始值, -32768~32767 对应 -180°~180°)
...
{      
    ...
    {  
        ...
        if(imu_rx_cnt == 0 && res == 0x55) imu_rx_cnt = 1;
        else if(imu_rx_cnt == 1 && res == 0x53) imu_rx_cnt = 2;
        else if(imu_rx_cnt >= 2 && imu_rx_cnt < 11)
        {
            RxBuffer[imu_rx_cnt++] = res;
            if(imu_rx_cnt == 11)
            {
                // 解析角度数据：第7,8字节是航向角(原始值, 直接存整数, 中断里不做浮点)
                Yaw = (short)((RxBuffer[7] << 8) | RxBuffer[6]);
                imu_rx_cnt = 0; // 清零，等待下一包
            }
        }
        else imu_rx_cnt = 0; // 数据不对，重置计数
...
uint8_t JY_9AXIS[]     = {0xFF, 0xAA, 0x24, 0x02, 0x00}; // 9轴模式(磁力计修正, 不漂移)
...
void JY61P_Full_Init_Sequence(void) {
    ...
    // 切到9轴模式(磁力计修正航向), 避免6轴模式积分漂移导致转弯测不准
    JY61P_Send_Cmd(JY_UNLOCK, 5);      // 解锁
    delay_ms(100);
    JY61P_Send_Cmd(JY_9AXIS, 5);       // 9轴模式
    delay_ms(100);
    JY61P_Send_Cmd(JY_SAVE, 5);        // 保存
    delay_ms(200);
}
